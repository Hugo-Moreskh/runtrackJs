...
function afficherModale() {
  ...
  // Afficher la modale avec le contenu construit
  alert(contenuModale);
}

// Variables pour suivre l'ordre des touches pressées
var touchesPressees = "";

// Fonction pour détecter les touches pressées
document.addEventListener("keydown", function (event) {
  // Ajouter la touche pressée à la variable
  touchesPressees += event.key;

  // Vérifier si l'ordre D, G, C a été pressé
  if (touchesPressees === "DGC") {
    // Afficher la modale
    afficherModale();

    // Réinitialiser la variable pour permettre une nouvelle séquence
    touchesPressees = "";
  } else if (touchesPressees.length > 3) {
    // Si plus de 3 touches ont été pressées, réinitialiser la variable
    touchesPressees = "";
  }
});
